...
HOST = '127.0.0.1' #for using it over the internet, specify your private IP(from ipconffig or ifconfig)
...
def receive():
    while True:
        client, address = server.accept()   #The server has to accept new connections continuously
        ...
        print(f"Nickname of the client is: {nickname}") #Displaying the nickname of the client
        ...
        thread = threading.Thread(target=handle, args=(client,)) #The reason for comma is that it requires a tuple
        thread.start()
